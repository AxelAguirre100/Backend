const getCart = async () => {
    ...
    if (cartProducts.length == 0) {
        ...
        cartProducts.forEach(product => {
            ...
            if (productPopulate && productPopulate.price) {
            ...
            document.getElementById(`sumarQuantity${productPopulate._id}`).addEventListener('click', async () => {
                const sumarQuantityResponse = await fetch(`${url}/api/cart/product/${productPopulate._id}`, {
                    method: 'PUT',
                    headers: {
            ...
            document.getElementById(`restarQuantity${productPopulate._id}`).addEventListener('click', async () => {
                ...
                const restarQuantityResponse = await fetch(`${url}/api/cart/product/${productPopulate._id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
            ...
            document.getElementById(`btnBorrar${productPopulate._id}`).addEventListener("click", async () => {
                const deleteProductFromCartResponse = await fetch(`${url}/api/cart/product/${productPopulate._id}`, {
                    method: 'DELETE',
                })
...
document.getElementById('checkoutButton').addEventListener('click', async (e) => {
    e.preventDefault();
    try {
        const response = await fetch(`${url}/api/cart/purchase`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        if (response.ok) {
            ...
            Swal.fire({
                position: 'top-end',
                icon: 'error',
                title: `Error: ${responseBody.message}`,
                showConfirmButton: false,
                timer: 1500
            });
        }
